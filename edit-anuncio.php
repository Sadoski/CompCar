...
session_start();
    ...
    if ($foto ==''){
        ...
        $anuncio->updateAnuncio($marca,$modelo,$combustivel,$cor,$tipo,$km,$ano,$val,$adicionais,$id);
        
    }else{
        $image = image();
        $val = moeda($valor);
        $anuncio = new Anuncios();
        $anuncio->updateAnuncioFoto($marca,$modelo,$combustivel,$cor,$tipo,$km,$ano,$val,$adicionais,$image,$id);
    }
